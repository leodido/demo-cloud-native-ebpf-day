#include "utils.c"
#include <unistd.h>
#include "restrict_connect.skel.h"

int main(int argc, char **argv)
{
    struct restrict_connect_bpf *skel;
    int err;
    ...
    skel = restrict_connect_bpf__open_and_load();
    if (!skel)
    {
    ...
    err = restrict_connect_bpf__attach(skel);
    if (err)
    {
...
cleanup:
    restrict_connect_bpf__destroy(skel);
    return -err;
}
