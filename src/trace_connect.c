#include "utils.c"
#include "trace_connect.skel.h"

int main(int argc, char **argv)
{
    struct trace_connect_bpf *skel;
    int err;
    ...
    skel = trace_connect_bpf__open_and_load();
    if (!skel)
    {
    ...
    err = trace_connect_bpf__attach(skel);
    if (err)
    {
...
cleanup:
    trace_connect_bpf__destroy(skel);
    return -err;
}
